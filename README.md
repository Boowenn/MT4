...
- includes a structured AI/Governance Feedback layer in `QuantGod_GovernanceAdvisor.json` plus a Dashboard panel that explains, per route, why the Advisor recommends keep/demote/iterate/promote, where the risks are, and which parameter tests should run next
...
The parser writes `QuantGod_ParamLabResults.json` and `QuantGod_ParamLabResultsLedger.csv`, then annotates `QuantGod_ParamOptimizationPlan.json` with `resultScore`, `grade`, `promotionReadiness`, PF, win rate, net profit, trade count, and drawdown. Pending reports remain `PENDING_REPORT` and do not count as promotion evidence.
...
- an AI/Governance Feedback panel that turns each route action into `why / risk / next parameter tests` so operators can see the reasoning before any promotion or demotion decision
