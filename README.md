...
- includes a ParamLab Run History / Recovery ledger that writes `QuantGod_ParamLabRunRecovery.json` / `QuantGod_ParamLabRunRecoveryLedger.csv`; it shows every ParamLab guarded/config run by `runId`, terminal exit code, report missing/parsed/malformed counts, retry count, stop reason, and the next safe recovery action before any rerun
...
- a Run History / Recovery panel that shows ParamLab runId, mode, terminal exit code, report missing/parsed/malformed counts, retry count, stop reason, recovery action, and the archived status path
