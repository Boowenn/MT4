...
The parser writes `QuantGod_ParamLabResults.json` and `QuantGod_ParamLabResultsLedger.csv`, then annotates `QuantGod_ParamOptimizationPlan.json` with `resultScore`, `grade`, `promotionReadiness`, PF, win rate, net profit, trade count, and drawdown. `build_strategy_version_registry.bat` snapshots each route into a stable parameter version, `build_optimizer_v2_plan.bat` proposes the next tester-only generation against those parent versions, and `build_version_promotion_gate.bat` dry-runs promotion/demotion by `versionId`. Pending reports remain `PENDING_REPORT` and do not count as promotion evidence.
