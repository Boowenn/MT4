...
- keeps HFM live pilot mode at `0.01` lot with one-position caps, hard `SL/TP`, kill switches, a USD high-impact news filter, a 180-minute auto-resume cooldown after consecutive-loss pauses, a demoted-route exit guard for old live positions whose route has since been returned to candidate/simulation, and a startup entry guard that waits at least 15 minutes and the next H1 bar before any fresh order after EA restart
- ports the old MT4 `RSI_Reversal`, `BB_Triple`, `MACD_Divergence`, and `SR_Breakout` signal routes into MT5 as candidate/backtest/live-gated routes; the shipped HFM live preset enables `USDJPY RSI_Reversal H1` only among legacy routes, while `BB_Triple`, `MACD_Divergence`, and `SR_Breakout` stay candidate/backtest-only and require the non-RSI authorization lock plus exact live tag before any future live test
- adds RSI-specific fast exit protection for the live USDJPY RSI route: RSI-managed positions can move to breakeven after 10 minutes and 5 pips of favorable movement, then trail after 8 pips with a 3.5-pip distance. It also adds RSI failfast direct close / SL-tightening, a 90-minute time stop, cross-server-day close, SELL blocks in uptrend / tight-range conditions, and the startup entry guard. These guards are scoped to `QG_RSI_Rev` comments; RSI is intentionally governed by `EnablePilotRsiH1Live`, while the non-RSI authorization lock applies only to BB/MACD/SR.
...
- includes a QuantDinger-style MT5 Backend Backtest Loop: `tools/run_mt5_backend_backtest_loop.py` reads tester-only scheduler/optimizer candidates, loads MT5 bars through read-only `copy_rates_range` or an offline fixture, simulates MA/RSI/BB/MACD/SR routes in Python, writes `QuantGod_MT5BackendBacktest.json` / ledgers, exposes `/api/mt5-backtest-loop`, and feeds Governance Advisor as a scalable research pre-screen. It does not launch Strategy Tester, send orders, close/cancel positions, select symbols, store credentials, or mutate the live preset; see `docs/MT5_Backend_Backtest_Loop.md`.
- includes a guarded MT5 trading bridge and ticket operation surface: `/api/mt5/status`, `/profiles`, `/profile`, `/login`, `/order`, `/close`, `/cancel`, and `DELETE /api/mt5/order/<ticket>` call `tools/mt5_trading_client.py`. They are implemented but locked by default with `dryRun=true`, `killSwitch=true`, `ownerMode=EA_ONLY`, `orderSendAllowed=false`, no password persistence, no live preset mutation, and mandatory audit rows before any possible broker mutation; see `docs/MT5_Trading_Control_Contracts.md`.
- includes a unified live-trading factory in `tools/live_trading_factory.py`: `create_client("MT5", market_category="Forex")` returns a guarded MT5 client with `place_market_order`, `place_limit_order`, `close_position`, and `cancel_order`, all still routed through the same lock/audit bridge.
- includes a guarded MT5 pending-order worker: `tools/mt5_pending_order_worker.py` reads `QuantGod_MT5PendingOrderIntents.json`, derives idempotent intent IDs, validates pending-order-only tickets, calls the guarded trading bridge, writes `QuantGod_MT5PendingOrderWorker.json` plus `QuantGod_MT5PendingOrderLedger.csv`, and skips duplicate accepted intents. It defaults to dry-run and still requires the same authorization lock for live placement.
- includes a local product/control-plane store: `tools/mt5_platform_store.py` writes `QuantGod_MT5Platform.db` and `QuantGod_MT5PlatformState.json`, tracking operators, role permissions, product features, task runs, and synchronized MT5 trading/worker audit events. It is a SQLite-backed local platform layer and never sends orders.
- includes a guarded MT5 adaptive-control executor: `tools/mt5_adaptive_control_executor.py` reads Governance Advisor / Version Promotion Gate decisions, writes `QuantGod_MT5AdaptiveControlActions.json`, `QuantGod_MT5AdaptiveControlLedger.csv`, and optional `QuantGod_MT5AdaptiveControlStaging.set`; live preset mutation is disabled by default and requires a separate env/config/auth-lock gate.
...
- Phase 2 now has the missing MT5 platform structure in place. Remaining work is evidence quality and operational hardening: keep adaptive controls advisory until promotion evidence improves, keep pending/live mutation locked unless a human supplies the signed authorization lock, and continue expanding labeling/linkage/regime reports where they feed research or explicitly gated ticket operations.

Important: the current MT5 implementation is still a guarded pilot. In the shipped HFM live preset, real-money automation is limited to `USDJPY RSI_Reversal H1`; `MA_Cross` is disabled in the preset, and `BB_Triple H1`, `MACD_Divergence H1`, and `SR_Breakout M15` are candidate/backtest iteration routes. BB/MACD/SR cannot send live orders unless their explicit live switch is re-enabled, `EnableNonRsiLegacyLiveAuthorization=true`, and `NonRsiLegacyLiveAuthorizationTag=ALLOW_NON_RSI_LEGACY_LIVE` after evidence and human approval.
...
The Vue workbench lives at `http://localhost:8080/vue/`. It is now the only active operator frontend: MT5, Polymarket, ParamLab, trend charts, and reports are separated into app-style workspaces, and the retired `QuantGod_Dashboard.html` route redirects to `/vue/`. The Vue shell includes deep detail components for MT5 positions/governance/backtests, Polymarket radar/AI/canary/governance/linkage/history, ParamLab queue/results/recovery evidence, route/status/sort controls for ParamLab batch review, evidence freshness and report tables, and a read-only chart/trend workspace for MT5 Shadow/Candidate outcomes, ParamLab score trends, Report Watcher, Run Recovery, AutoTester gate, research slices, and Polymarket radar/AI/canary/governance distributions. The retirement status is tracked in `docs/QuantGod_Dashboard_Vue_Migration_Status.md`.
...
Dashboard/vue-dist/                      -> [HFM MT5]/MQL5/Files/vue-dist/
...
- open `http://localhost:8080/vue/` with a cache-busting timestamp
...
Then open the Vue workbench:

```text
http://localhost:8080/vue/
```

The retired single-file dashboard route now redirects to Vue:
