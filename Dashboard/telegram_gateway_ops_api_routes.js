...
function isTelegramGatewayOpsPath(requestUrl) {
  const pathname = String(requestUrl || '').split('?')[0];
  return pathname === '/api/telegram-gateway' || pathname.startsWith('/api/telegram-gateway/');
}
...
function runPythonJson(repoRoot, args, timeoutMs = 120000) {
  return new Promise((resolve) => {
    ...
    const script = path.join(repoRoot, 'tools', 'run_telegram_gateway_ops.py');
...
async function handle(req, res, ctx) {
  ...
  if (req.method === 'POST' && pathname === '/api/telegram-gateway/collect') {
    const payload = await runPythonJson(ctx.repoRoot, ['--runtime-dir', runtimeDir, 'collect']);
    sendJson(res, statusCodeFor(payload), payload);
    return;
  }
