...
- Remember that the overview section now also contains a local-time `昨晚 vs 今天` research summary card, with windows split as `昨天 20:00 -> 今天 08:00` and `今天 08:00 -> 现在`.
...
- Check the `昨晚 vs 今天` summary card whenever you change trade timestamp parsing, local-time handling, or the closed-trade aggregation path.
