...
- Treat `QuantGod_GovernanceAdvisor.json`, `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, `QuantGod_VersionPromotionGate.json`, and `QuantGod_ParamLabAutoScheduler.json` as derived local advisory files from those runtime files. They help summarize route lifecycle state, strategy versions, tester-only optimization proposals, dry-run promotion/demotion decisions, and the next config-only ParamLab queue, but they are not live execution paths.
...
- For promotion/demotion or route-quality questions, also inspect or regenerate `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, `QuantGod_VersionPromotionGate.json`, `QuantGod_ParamLabAutoScheduler.json`, and `QuantGod_GovernanceAdvisor.json` with `tools\build_strategy_version_registry.py`, `tools\build_optimizer_v2_plan.py`, `tools\build_version_promotion_gate.py`, `tools\build_param_lab_auto_scheduler.py`, and `tools\build_governance_advisor.py`.
...
- For Governance Advisor, Strategy Version Registry, Optimizer V2, Version Promotion Gate, or ParamLab Auto Scheduler changes, run `python tools\build_strategy_version_registry.py`, `python tools\build_optimizer_v2_plan.py`, `python tools\build_version_promotion_gate.py`, `python tools\build_param_lab_auto_scheduler.py`, and `python tools\build_governance_advisor.py`, verify the five JSON files under the HFM Files directory, and check the dashboard Strategy Workspace / Governance Advisor / Auto Scheduler cards. These advisors must remain file-only, must not add a new broker/order path or mutate the live preset, and must not add `-RunTerminal` by default.
