...
- Treat `QuantGod_GovernanceAdvisor.json`, `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, `QuantGod_VersionPromotionGate.json`, `QuantGod_ParamLabAutoScheduler.json`, and `QuantGod_ParamLabReportWatcher.json` as derived local advisory files from those runtime files. They help summarize route lifecycle state, strategy versions, tester-only optimization proposals, dry-run promotion/demotion decisions, the next config-only ParamLab queue, and landed tester-report parsing state, but they are not live execution paths.
...
- For promotion/demotion or route-quality questions, also inspect or regenerate `QuantGod_ParamLabReportWatcher.json`, `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, `QuantGod_VersionPromotionGate.json`, `QuantGod_ParamLabAutoScheduler.json`, and `QuantGod_GovernanceAdvisor.json` with `tools\watch_param_lab_reports.py`, `tools\build_strategy_version_registry.py`, `tools\build_optimizer_v2_plan.py`, `tools\build_version_promotion_gate.py`, `tools\build_param_lab_auto_scheduler.py`, and `tools\build_governance_advisor.py`.
...
- Remember that the QuantDinger-inspired Governance Advisor, Strategy Version Registry, Optimizer V2, Version Promotion Gate, ParamLab Auto Scheduler, Report Watcher, and light dashboard shell are file-only and advisory: they combine Backtest Lab, live forward, Shadow/Outcome, Candidate/Outcome, Manual Alpha, local runtime-health evidence, strategy parameter hashes, parent/child version lineage, tester-only next-generation proposals, dry-run promotion/demotion recommendations, config-only tester queues, and landed Strategy Tester reports into `QuantGod_GovernanceAdvisor.json`, `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, `QuantGod_VersionPromotionGate.json`, `QuantGod_ParamLabAutoScheduler.json`, and `QuantGod_ParamLabReportWatcher.json`, but must never store credentials, connect to HFM, send orders, mutate the HFM live preset, add `-RunTerminal` by default, or bypass EA `OrderSend` gating.
...
- For Governance Advisor, Strategy Version Registry, Optimizer V2, Version Promotion Gate, ParamLab Auto Scheduler, or Report Watcher changes, run `python tools\watch_param_lab_reports.py`, `python tools\build_strategy_version_registry.py`, `python tools\build_optimizer_v2_plan.py`, `python tools\build_version_promotion_gate.py`, `python tools\build_param_lab_auto_scheduler.py`, and `python tools\build_governance_advisor.py`, verify the derived JSON files under the HFM Files directory, and check the dashboard Strategy Workspace / Governance Advisor / Auto Scheduler / Report Watcher cards. These advisors must remain file-only, must not add a new broker/order path or mutate the live preset, and must not add `-RunTerminal` by default.
