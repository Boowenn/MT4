...
- Treat `QuantGod_GovernanceAdvisor.json` as a derived local advisor from those runtime files. It helps summarize route lifecycle state, but it is not an execution path.
...
- For promotion/demotion or route-quality questions, also inspect or regenerate `QuantGod_GovernanceAdvisor.json` with `tools\build_governance_advisor.py`.
...
- Remember that the QuantDinger-inspired Governance Advisor and light dashboard shell are file-only and advisory: they combine Backtest Lab, live forward, Shadow/Outcome, Candidate/Outcome, Manual Alpha, and local runtime-health evidence into `QuantGod_GovernanceAdvisor.json`, but must never store credentials, connect to HFM, send orders, or bypass EA `OrderSend` gating.
...
- For Governance Advisor changes, run `python tools\build_governance_advisor.py`, verify `QuantGod_GovernanceAdvisor.json` under the HFM Files directory, and check the dashboard `Governance Advisor` card. The advisor must remain file-only and must not add a new broker/order path.
