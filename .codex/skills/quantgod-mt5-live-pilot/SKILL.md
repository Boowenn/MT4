...
- Treat `QuantGod_GovernanceAdvisor.json`, `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, and `QuantGod_VersionPromotionGate.json` as derived local advisory files from those runtime files. They help summarize route lifecycle state, strategy versions, tester-only optimization proposals, and dry-run promotion/demotion decisions, but they are not execution paths.
...
- For promotion/demotion or route-quality questions, also inspect or regenerate `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, `QuantGod_VersionPromotionGate.json`, and `QuantGod_GovernanceAdvisor.json` with `tools\build_strategy_version_registry.py`, `tools\build_optimizer_v2_plan.py`, `tools\build_version_promotion_gate.py`, and `tools\build_governance_advisor.py`.
...
- Remember that the QuantDinger-inspired Governance Advisor, Strategy Version Registry, Optimizer V2, Version Promotion Gate, and light dashboard shell are file-only and advisory: they combine Backtest Lab, live forward, Shadow/Outcome, Candidate/Outcome, Manual Alpha, local runtime-health evidence, strategy parameter hashes, parent/child version lineage, tester-only next-generation proposals, and dry-run promotion/demotion recommendations into `QuantGod_GovernanceAdvisor.json`, `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, and `QuantGod_VersionPromotionGate.json`, but must never store credentials, connect to HFM, send orders, mutate the HFM live preset, or bypass EA `OrderSend` gating.
...
- For Governance Advisor, Strategy Version Registry, Optimizer V2, or Version Promotion Gate changes, run `python tools\build_strategy_version_registry.py`, `python tools\build_optimizer_v2_plan.py`, `python tools\build_version_promotion_gate.py`, and `python tools\build_governance_advisor.py`, verify the four JSON files under the HFM Files directory, and check the dashboard Strategy Workspace / Governance Advisor cards. These advisors must remain file-only and must not add a new broker/order path or mutate the live preset.
