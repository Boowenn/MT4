...
- Remember that the QuantDinger-inspired Governance Advisor is file-only and advisory: it combines Backtest Lab, live forward, Shadow/Outcome, Candidate/Outcome, and Manual Alpha evidence into `QuantGod_GovernanceAdvisor.json`, but must never store credentials, connect to HFM, send orders, or bypass EA `OrderSend` gating.
