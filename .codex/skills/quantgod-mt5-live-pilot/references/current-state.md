...
Treat `QuantGod_GovernanceAdvisor.json`, `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, `QuantGod_VersionPromotionGate.json`, and `QuantGod_ParamLabAutoScheduler.json` as derived advisory snapshots from those files, not as direct live execution sources.
...
- Provide a controlled ParamLab Runner for those offline candidates: `tools/run_param_lab.py` materializes ranked candidates into tester-only `.set` and `.ini` files, archives them under `archive/param-lab/runs/`, writes `QuantGod_ParamLabStatus.json`, and can parse reports back into the plan after an authorized Strategy Tester run. Its default mode is config-only and it must not overwrite the live preset.
- Provide a ParamLab Results Parser for completed tester reports: `tools/collect_param_lab_results.py` scans ParamLab run archives, parses PF, win rate, net profit, trade count, max drawdown, relative drawdown, writes `QuantGod_ParamLabResults.json` plus `QuantGod_ParamLabResultsLedger.csv`, and annotates `QuantGod_ParamOptimizationPlan.json` with result scores. Pending reports stay advisory `PENDING_REPORT` rows and cannot promote a route.
...
- Provide a QuantDinger-inspired Governance Advisor, Strategy Version Registry, Optimizer V2, Version Promotion Gate, ParamLab Auto Scheduler, and light dashboard shell as a local, file-only lifecycle snapshot. It combines Backtest Lab, live `0.01` forward results, Shadow Signal/Outcome, Shadow Candidate/Outcome, Manual Alpha, runtime file-health evidence, parameter hashes, version-linked tester-only optimizer proposals, dry-run promotion/demotion recommendations, and config-only tester queues into `QuantGod_GovernanceAdvisor.json`, `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, `QuantGod_VersionPromotionGate.json`, and `QuantGod_ParamLabAutoScheduler.json`; it is advisory only and never connects to HFM, stores credentials, sends orders, mutates the HFM live preset, adds `-RunTerminal` by default, or bypasses EA `OrderSend` gating
...
- `tools/run_param_lab.py` is the durable local implementation of the controlled ParamLab runner. It can create Strategy Tester candidate presets/configs and update `QuantGod_ParamLabStatus.json`; by default it picks a route-balanced set so RSI/BB/MACD/SR each get a top candidate before extra slots are filled by score. Launching MT5 requires `--run-terminal --authorized-strategy-tester` and the weekend/explicit authorization boundary. Parsed ParamLab results remain advisory evidence and are not written into the live preset automatically.
...
- It does not automatically mutate live strategy parameters. Parameter self-optimization is staged through `QuantGod_ParamOptimizationPlan.json`, `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, `QuantGod_VersionPromotionGate.json`, `QuantGod_ParamLabAutoScheduler.json`, and Governance Advisor review before any preset change is considered.
...
- Use `QuantGod_GovernanceAdvisor.json`, `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, `QuantGod_VersionPromotionGate.json`, and `QuantGod_ParamLabAutoScheduler.json` when the question is about promotion, demotion, strategy version lineage, or which route should stay live versus simulation; regenerate them first if the runtime evidence has changed.
