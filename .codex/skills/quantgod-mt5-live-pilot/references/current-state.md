...
- Render a read-only AI/Governance Feedback panel from `QuantGod_GovernanceAdvisor.json`, showing each route's `why`, structured risk areas, and next parameter tests. This is explanation and planning only; it does not promote routes, launch Strategy Tester, or write live configuration.
