...
- Port the old MT4 `RSI_Reversal`, `BB_Triple`, `MACD_Divergence`, and `SR_Breakout` routes into MT5 as live-gated signal evaluators. The shipped HFM live preset now enables all four legacy live switches for faster `0.01` forward sampling, while preserving the same pilot-only one-position cap and shared spread/session/news/cooldown/portfolio/order-send/hard-SL/kill-switch controls. Strategy Tester presets still validate the same route family in controlled backtests.
...
- The legacy MT4 routes are no longer pure dashboard placeholders on MT5. They expose live-gated runtime status, and all four route live switches are enabled in the HFM live preset. Every route still must pass the shared `0.01`, single-position, spread/session/news/cooldown/portfolio/order-send, hard-SL, and kill-switch controls before sending an order.
...
- In HFM live-pilot mode, `MA_Cross`, `USDJPY RSI_Reversal H1`, `BB_Triple H1`, `MACD_Divergence H1`, and `SR_Breakout M15` are real executable live-gated slices enabled by the shipped live preset. They remain constrained to `0.01`, a single total pilot position, and the shared protective controls.
