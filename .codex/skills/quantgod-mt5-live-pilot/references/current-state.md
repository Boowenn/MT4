...
- Port the old MT4 `RSI_Reversal`, `BB_Triple`, `MACD_Divergence`, and `SR_Breakout` routes into MT5 as candidate/backtest/live-gated signal evaluators. The shipped HFM live preset keeps their live switches off until validation, while Strategy Tester presets can enable them for controlled backtests.
...
- The legacy MT4 routes are no longer pure dashboard placeholders on MT5. They expose candidate/backtest/live-gated runtime status, and can only send real orders if their explicit route live switch is enabled together with the shared `0.01`, single-position, spread/session/news/cooldown/portfolio/order-send, hard-SL, and kill-switch controls.
...
- In HFM live-pilot mode, `MA_Cross` is the only real executable slice enabled by the shipped live preset; the remaining MT5 strategies are ported routes with live switches disabled by default pending validation.
