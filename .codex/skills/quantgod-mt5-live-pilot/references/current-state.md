...
- Provide ParamLab Run History / Recovery: `tools/build_param_lab_run_recovery.py` writes `QuantGod_ParamLabRunRecovery.json` plus `QuantGod_ParamLabRunRecoveryLedger.csv`, summarizing guarded/config runs by runId, terminal exit code, report missing/parsed/malformed state, retry count, stop reason, and recovery action. It is read-only and never launches MT5.
...
- Render a read-only Run History / Recovery panel from `QuantGod_ParamLabRunRecovery.json`, showing runId, mode, terminal exit code, report missing/parsed/malformed counts, retry count, stop reason, recovery action, and archived status path.
...
- `tools/build_param_lab_run_recovery.py` is the durable local implementation of guarded-run recovery visibility. It reads ParamLab run archives, AUTO_TESTER_WINDOW status, and report watcher results, then writes run-level stop reasons and recovery actions without launching MT5 or mutating live configuration.
