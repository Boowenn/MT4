...
Treat `QuantGod_GovernanceAdvisor.json`, `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, `QuantGod_VersionPromotionGate.json`, `QuantGod_ParamLabAutoScheduler.json`, and `QuantGod_ParamLabReportWatcher.json` as derived advisory snapshots from those files, not as direct live execution sources.
...
- It does not automatically mutate live strategy parameters. Parameter self-optimization is staged through `QuantGod_ParamOptimizationPlan.json`, `QuantGod_ParamLabReportWatcher.json`, `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, `QuantGod_VersionPromotionGate.json`, `QuantGod_ParamLabAutoScheduler.json`, and Governance Advisor review before any preset change is considered.
...
- Use `QuantGod_GovernanceAdvisor.json`, `QuantGod_ParamLabReportWatcher.json`, `QuantGod_StrategyVersionRegistry.json`, `QuantGod_OptimizerV2Plan.json`, `QuantGod_VersionPromotionGate.json`, and `QuantGod_ParamLabAutoScheduler.json` when the question is about promotion, demotion, strategy version lineage, or which route should stay live versus simulation; regenerate them first if the runtime evidence has changed.
