...
class AutonomousLifecycleTests(unittest.TestCase):
    ...
    def test_daily_autopilot_v2_summarizes_three_lanes_without_execution(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(payload["agentVersion"], "v2.5")
            ...
            self.assertFalse(payload["nextPhaseTodos"]["completedByAgent"])
            self.assertEqual(payload["nextPhaseTodos"]["strategyJsonTodo"]["status"], "WAITING_NEXT_PHASE")
