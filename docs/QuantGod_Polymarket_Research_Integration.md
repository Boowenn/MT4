...
`/api/polymarket/search` is a unified search facade over the history DB, current opportunity radar, latest single-market analysis/history, and AI score snapshot. It returns compact evidence cards plus per-source counts so the Dashboard can search market/question/route/risk evidence from one box instead of separate panels.
...
- Unified Evidence Search: `/api/polymarket/search` aggregates history, radar, single-market analysis, and AI score evidence into one read-only Dashboard query box.
